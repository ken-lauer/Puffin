...
# * Copyright (c) 2013-2018, Lawrence Campbell and Brian McNeil. **
...
for slice in filelist:
    ...
    sumData[fieldCount] = numpy.trapz(
        h5in.root._f_getChild(datasetname).read(), None, deltaz2
    )
    peakData[fieldCount] = numpy.max(h5in.root._f_getChild(datasetname).read())
    fieldNormData[:, fieldCount] = (
        h5in.root._f_getChild(datasetname).read() / peakData[fieldCount]
    )
    h5in.close()
    fieldCount += 1
...
for fieldname in [datasetname + "_ST", datasetname + "_ST_Norm"]:
    ...
    h5.root._v_children[fieldname]._v_attrs.time = 0.0
...
for fieldname in [datasetname + "_integral", datasetname + "_peak"]:
    h5.root._v_children[fieldname]._v_attrs.vsMesh = "zSeries"
    h5.root._v_children[fieldname]._v_attrs.vsType = "variable"
