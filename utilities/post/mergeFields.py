...
if len(sys.argv) != 2:
    ...
    print(
        "Please provide full filename, or a basename without extension to match multiple files"
    )
    exit()
filename = sys.argv[1]
...
if isvsh5suffix(filename):
    ...
    if len(filenamepieces) == 6:
        print("We are here")
        (simbase, dataset, fieldpart, dsetcomponent, dsetrank, dumpnoAndExt) = (
            filenamepieces
        )
    ...
    #  outfilename = filename[:-5]+"C.vsh5"
        ...
        print(
            "unknown file format - don't know how to tell dumps, rank, dataset from filename"
        )
        exit()
    tdsetname = dataset + "N"
...
for fname in fileList:
    ...
    if g:
        ...
        thisNZ2[dsetname + "_" + thisRank] = (
            t1.root._v_children[dsetname].read().shape[0]
        )
        ...
        for rank in range(0, maxRank + 1):
            ...
            if os.path.isfile(loopfname):
                ...
                if component == "real":
                    ...
                    fieldout[:, :, NZ2start : (NZ2start + thisNZ2[fieldNameR]), 0] = (
                        f1.root._v_children[dataset + "_" + fieldPos + "_" + component]
                        .read()
                        .T
                    )
                ...
                if component == "imag":
                    fieldout[:, :, NZ2start : (NZ2start + thisNZ2[fieldNameR]), 1] = (
                        f1.root._v_children[dataset + "_" + fieldPos + "_" + component]
                        .read()
                        .T
                    )
...
h5.root._v_children["meshScaled"]._v_attrs.vsNumCells = numpy.array(
    (NX - 1, NY - 1, NZ2 - 1)
)
