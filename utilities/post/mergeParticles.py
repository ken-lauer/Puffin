...
def getNumProcs(filelist, baseName, n):
    ...
    thisDump = int(thisFile.split(os.sep)[-1].split(".")[0].split("_")[-1])
    procPtclFiles = glob.glob(
        os.getcwd() + os.sep + baseName + "_*_" + str(thisDump) + ".h5"
    )
    print(procPtclFiles)
...
for i in range(len(filelist)):
    ...
    for j in range(numProcs0):
        if j == 0:
            ...
            h5in = tables.open_file(
                baseName + "_" + str(j) + "_" + str(dumpStepNos[i]) + ".h5", "r"
            )
    ...
    for j in range(1, numProcs0):
        ...
        elecs[particleCount : particleCount + h5in.root.electrons.shape[0], :] = (
            newElecs
        )
        particleCount += h5in.root.electrons.shape[0]
        h5in.close()
