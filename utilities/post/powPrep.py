...
h5.root.gridZ_SI._v_attrs.vsLowerBounds = numpy.array(
    (numpy.double(minS), numpy.double(minZT))
)
h5.root.gridZ_SI._v_attrs.vsStartCell = numpy.array((numpy.int(0), numpy.int(0)))
h5.root.gridZ_SI._v_attrs.vsUpperBounds = numpy.array(
    (numpy.double(maxS), numpy.double(maxZT))
)
...
if minZZ is not None:
    ...
    h5.root.gridZScaled._v_attrs.vsLowerBounds = numpy.array(
        (numpy.double(minS), numpy.double(minZZ))
    )
    h5.root.gridZScaled._v_attrs.vsStartCell = numpy.array((numpy.int(0), numpy.int(0)))
    h5.root.gridZScaled._v_attrs.vsUpperBounds = numpy.array(
        (numpy.double(maxS), numpy.double(maxZZ))
    )
...
for slice in filelist:
    ...
    sumData[fieldCount] = numpy.trapz(
        h5in.root._f_get_child(datasetname).read(), None, deltaz2
    )
    ...
    if peakData[fieldCount] != 0:
        fieldNormData[:, fieldCount] = (
            h5in.root._f_get_child(datasetname).read() / peakData[fieldCount]
        )
    else:
...
h5.root.time._v_attrs.vsTime = 0.0
...
z2Data = numpy.linspace(
    numpy.double(minS), numpy.double(maxS), numpy.int(numSpatialPoints)
)
...
h5.root.gridTPowEv._v_attrs.vsLowerBounds = numpy.array(
    (numpy.double(zData[0]), numpy.double(z2Data[0]))
)
h5.root.gridTPowEv._v_attrs.vsUpperBounds = numpy.array(
    (numpy.double(zData[-1]), numpy.double(z2Data[-1]))
)
if qScale == 0:
