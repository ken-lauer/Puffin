...
def reduceField(fname, oname):
    ...
    if ninner != 1:
        ...
        xno = (nx - newnx) / 2
        yno = (ny - newny) / 2
        lowLimX = -dx * (newnx - 1.0) / 2.0
        lowLimY = -dy * (newny - 1.0) / 2.0

        uppLimX = dx * (newnx - 1.0) / 2.0
        uppLimY = dy * (newny - 1.0) / 2.0
    ...
    if ninner == 1:
        ...
        redField = h5f.root.aperp[
            0:2, :, xno + 1 : nx - xno + 1, yno + 1 : ny - yno + 1
        ]  # slice out our reduced field
    ...
    if ninner == 1:
        ...
        h5o.root.meshScaled._v_attrs.vsUpperBounds = dz2 * (nz2 - 1.0)
    ...
    #        h5o.root.globalLimits._v_attrs.vsUpperBounds = numpy.array((dz2 * (nz2 - 1.)))
        ...
        h5o.root.meshScaled._v_attrs.vsUpperBounds = numpy.array(
            (uppLimX, uppLimY, dz2 * (nz2 - 1.0))
        )
