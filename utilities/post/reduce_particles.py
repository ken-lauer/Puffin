...
h5out = tables.open_file(
    "_".join(outfilebase) + "_reduced_" + dumpno + "." + fileextname, "w"
)
...
h5out.root.globalLimits._v_attrs["vsLowerBounds"] = numpy.array(
    (
        numpy.min(newparticles[:, 0]),
        numpy.min(newparticles[:, 1]),
        numpy.min(newparticles[:, 2]),
    )
)
h5out.root.globalLimits._v_attrs["vsUpperBounds"] = numpy.array(
    (
        numpy.max(newparticles[:, 0]),
        numpy.max(newparticles[:, 1]),
        numpy.max(newparticles[:, 2]),
    )
)
h5out.create_group("/", "time")
...
print("1D histogram")
H1D, edges1D = numpy.histogram(
    elecs[:, 2], bins=(numpy.linspace(lb[2], ub[2], 570)), weights=elecs[:, 6]
)
print(edges1D)
writeChargeOutput1D(H1D, edges1D, elecs, "charge1D")
Hreduced1D, edgesReduced1D = numpy.histogram(
    newparticles[:, 2], bins=(numpy.linspace(lb[2], ub[2], 570))
)
