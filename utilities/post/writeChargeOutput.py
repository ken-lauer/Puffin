...
def writeChargeOutput3D(filename, ptcldata, bins, simtime=0.0):
    print(ptcldata.shape)
    print(str(ptcldata.shape[0]) + " particles to be histogrammed")
    if ptcldata.shape[1] == 7:
        histdata, edgedata = numpy.histogramdd(
            ptcldata[:, 0:3], bins=bins, weights=ptcldata[:, 6]
        )
    ...
    h5out.root.globalLimits._v_attrs["vsLowerBounds"] = numpy.array(
        (numpy.min(edgedata[0]), numpy.min(edgedata[1]), numpy.min(edgedata[2]))
    )
    h5out.root.globalLimits._v_attrs["vsUpperBounds"] = numpy.array(
        (numpy.max(edgedata[0]), numpy.max(edgedata[1]), numpy.max(edgedata[2]))
    )
    ...
    h5out.root.mesh._v_attrs["vsLowerBounds"] = numpy.array(
        (numpy.min(edgedata[0]), numpy.min(edgedata[1]), numpy.min(edgedata[2]))
    )
    h5out.root.mesh._v_attrs["vsUpperBounds"] = numpy.array(
        (numpy.max(edgedata[0]), numpy.max(edgedata[1]), numpy.max(edgedata[2]))
    )
    h5out.root.mesh._v_attrs["vsNumCells"] = numpy.array(
        (len(edgedata[0]) - 1, len(edgedata[1]) - 1, len(edgedata[2]) - 1)
    )
...
def writeChargeOutput1D(
    histdata, edgedata, ptcldata, dataname, dumpno=0, fileextname="h5"
):
    h5out = tables.open_file(
        "_".join(outfilebase) + dataname + "_" + str(dumpno) + "." + fileextname, "w"
    )
...
def writeVsh53DParticlesSimple(
    filename, ptcldata, dsetname, simtime=0.0, step=0, lowerbounds=[], upperbounds=[]
):
    if lowerbounds == []:
        lowerbounds = numpy.array(
            (
                numpy.min(ptcldata[:, 0]),
                numpy.min(ptcldata[:, 1]),
                numpy.min(ptcldata[:, 2]),
            )
        )
    if upperbounds == []:
        numpy.array(
            (
                numpy.max(ptcldata[:, 0]),
                numpy.max(ptcldata[:, 1]),
                numpy.max(ptcldata[:, 2]),
            )
        )
    h5out = tables.open_file(filename, "w")
...
def main():
    ...
    writeVsh53DParticlesSimple(
        ("_".join(outfilebase) + "_reduced_" + dumpno + "." + fileextname),
        newparticles,
        "electrons_reduced",
        simtime,
    )
    ...
    writeChargeOutput3D(
        ("_".join(outfilebase) + "Charge3D_" + dumpno + "." + fileextname),
        elecs,
        (
            numpy.linspace(lb[0], ub[0], 32),
            numpy.linspace(lb[1], ub[1], 32),
            numpy.linspace(lb[2], ub[2], 228),
        ),
    )
    print("and writing reduced charge data")
    writeChargeOutput3D(
        ("_".join(outfilebase) + "Charge3DReduced_" + dumpno + "." + fileextname),
        newparticles,
        (
            numpy.linspace(lb[0], ub[0], 32),
            numpy.linspace(lb[1], ub[1], 32),
