...
def convertFile(filename):
    ...
    if isvsh5suffix(filename):
        ...
        print(
            "need to figure out if we're here, as there will be multiple output files"
        )
    print(outfilename)
    ...
    if len(fieldin.shape) == 4:
        fieldout = numpy.zeros(
            (fieldin.shape[3], fieldin.shape[2], fieldin.shape[1], fieldin.shape[0])
        )
        ...
        print(
            "Do not recognise structure of file "
            + filename
            + " setting the _C file as just a copy"
        )
