...
def getMagPhase(h5, rho, xi, yi, component):
    ...
    dz = zLen / zNumCells
    nElements = numpy.int(
        numpy.round(wavelength / dz)
    )  # in our 4 rho section for making an average, nel
    nNodes = nElements + 1  # nnl
    nElAverage = numpy.int(
        numpy.floor((nNodes) / 2.0)
    )  # df,  2*df is the number of nodes spanning a resonant wavelength
    ...
    for m in range(0, nz):
        ...
        if hi > (nz - 1):
            hi = nz - 1
        intxrms[m] = numpy.sqrt(
            numpy.mean(numpy.square(h5.root.aperp[xi, yi, lo:hi, component]))
        )
    ...
    z2cyclic = (
        numpy.subtract(
            zMesh, (4.0 * numpy.pi * rho * numpy.floor(zMesh / (4.0 * numpy.pi * rho)))
        )
    ) / (2.0 * rho)
    # slopy does not appear to be used
    # figure out some sort of instantaneous phase
    invcosx = numpy.arccos(
        numpy.divide(h5.root.aperp[xi, yi, :, component], numpy.max(magxrms, 1.0e-99))
    )
    for a in range(0, nz):
    ...
    for a in range(0, nz - 1):
        if (
            h5.root.aperp[xi, yi, a, component]
            < h5.root.aperp[xi, yi, (a + 1), component]
        ):
...
if len(sys.argv) == 4 or len(sys.argv) == 3:
    ...
    print("ny: " + str(ny))
    print("nz: " + str(nz))
    print(numpy.int((ny - 1) / 8.0))
    print(numpy.int(7 * (ny - 1) / 8.0) + 1)
    print(numpy.int(numpy.ceil((ny - 1) / 8.0)))
    ...
    for yi in range(
        numpy.int((ny - 1) / 8.0),
        numpy.int(7 * (ny - 1) / 8.0) + 1,
        numpy.int(numpy.ceil((ny - 1) / 8.0)),
    ):
        for xi in range(
            numpy.int((nx - 1) / 8.0),
            numpy.int(7 * (nx - 1) / 8.0) + 1,
            numpy.int(numpy.ceil((nx - 1) / 8.0)),
        ):
            plt.subplot(gs[count])
            mymax = max(
                numpy.max(numpy.abs(h5.root.aperp[xi, yi, : numpy.int(nz / 2), 0])),
                numpy.max(numpy.abs(h5.root.aperp[xi, yi, : numpy.int(nz / 2), 1])),
            )
            plt.scatter(
                h5.root.aperp[xi, yi, : (numpy.int(nz / 2)), 0] / mymax,
                h5.root.aperp[xi, yi, : (numpy.int(nz / 2)), 1] / mymax,
            )
    ...
    for xi in range(int(14 * nx / 32), int(18 * nx / 32)):
        for yi in range(int(14 * ny / 32), int(18 * ny / 32)):
            ...
            s0 = numpy.max(numpy.add(numpy.square(magx), numpy.square(magy)), 1.0e-99)
            s1 = numpy.subtract(numpy.square(magx), numpy.square(magy))
            s2 = 2 * numpy.multiply(
                numpy.multiply(magx, magy), numpy.cos(numpy.subtract(phasex, phasey))
            )
            s3 = 2 * numpy.multiply(
                numpy.multiply(magx, magy), numpy.sin(numpy.subtract(phasex, phasey))
            )
