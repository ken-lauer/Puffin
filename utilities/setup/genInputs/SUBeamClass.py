...
class SUBeam:
    def __init__(self, fname=false, SU_data=false):
        if not fname:
            if not SU_data:
                warnings.warn(
                    "SUBeam class must be initialized with a filename or particle data"
                )
            else:
    ...
    def calc_emittance(self):
        ...
        x_2 = ((np.sum(wts * m_POSx * m_POSx)) / np.sum(wts)) - (
            np.average(m_POSx, weights=wts)
        ) ** 2.0  #
        px_2 = ((np.sum(wts * m_mOmx * m_mOmx)) / np.sum(wts)) - (
            np.average(m_mOmx, weights=wts)
        ) ** 2.0  #
        xpx = (
            np.sum(wts * m_POSx * m_mOmx) / np.sum(wts)
            - np.sum(wts * m_POSx) * np.sum(wts * m_mOmx) / (np.sum(wts)) ** 2
        )  #
        #
        y_2 = ((np.sum(wts * m_POSy * m_POSy)) / np.sum(wts)) - (
            np.average(m_POSy, weights=wts)
        ) ** 2.0  #
        py_2 = ((np.sum(wts * m_mOmy * m_mOmy)) / np.sum(wts)) - (
            np.average(m_mOmy, weights=wts)
        ) ** 2.0  #
        ypy = (
            np.sum(wts * m_POSy * m_mOmy) / np.sum(wts)
            - np.sum(wts * m_POSy) * np.sum(wts * m_mOmy) / (np.sum(wts)) ** 2
        )  #
