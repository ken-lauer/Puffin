...
class bm:
    ...
    le = np.array([6.0, 6.0, 6.0, 6.0, 6.0, 6.0])
    nmps = np.ones(1) * 1
    qmatch = np.ones(1) * 1
    eratio = np.ones(1) * 1
    emitx = np.ones(1) * 1
    ...
    def wrbeam(self, f, no):
        ...
        f.write(
            "! sSigmaE - gaussian std dev in each dimension - x, y, z2, px, py, gamma.\n"
        )
        f.write(
        ...
        f.write("&NBLIST\n")
        f.write("nbeams = " + f"{self.nbeams:d}" + "\n")
        ...
        f.write("&BLIST\n")
        f.write("sSigmaE = " + ", ".join([f"{ij:.15E}" for ij in self.sig]) + "\n")
        f.write("sLenE = " + ", ".join([f"{ij:.15E}" for ij in self.le]) + "\n")
        f.write("iNumElectrons = " + ", ".join([f"{ij:d}" for ij in self.nmps]) + "\n")
        f.write("sQe = " + ", ".join([f"{ij:.15E}" for ij in self.Q]) + "\n")
        f.write("bcenter = " + ", ".join([f"{ij:.15E}" for ij in self.bcenz2]) + "\n")

        f.write("gammaf = " + ", ".join([f"{ij:.15E}" for ij in self.eratio]) + "\n")
        f.write("chirp = " + ", ".join([f"{ij:.15E}" for ij in self.chirp]) + "\n")
        f.write("mag = " + ", ".join([f"{ij:.15E}" for ij in self.bosc_Mag]) + "\n")
        f.write("fr = " + ", ".join([f"{ij:.15E}" for ij in self.bosc_Fr]) + "\n")
        f.write("emitx = " + ", ".join([f"{ij:.15E}" for ij in self.emitx]) + "\n")
        f.write("emity = " + ", ".join([f"{ij:.15E}" for ij in self.emity]) + "\n")
        f.write("alphax = " + ", ".join([f"{ij:.15E}" for ij in self.alphax]) + "\n")
        f.write("alphay = " + ", ".join([f"{ij:.15E}" for ij in self.alphay]) + "\n")
        f.write("qRndEj_G = " + ", ".join([torf(ij) for ij in self.qRoundEj]) + "\n")
        f.write("sSigEj_G = " + ", ".join([f"{ij:.15E}" for ij in self.sigEj]) + "\n")
        f.write("qMatched_A = " + ", ".join([torf(ij) for ij in self.qmatch]) + "\n")
        f.write("TrLdMeth = " + ", ".join([f"{ij:d}" for ij in self.trload]) + "\n")
        f.write("/\n")
