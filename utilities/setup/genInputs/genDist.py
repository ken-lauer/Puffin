...
lrscaled = 4.0 * np.pi * puffVars.rho  # scaled wavelength is 4 pi rho
...
gamma_central = (
    1.0 * puffVars.gamma0
)  # mean beam energy, expressed as the Lorentz factor
gammasigh = (
    0.001 * puffVars.gamma0
)  # rms energy spread  (in units of gamma, NOT gamma / gamma0!!!)

x_central = 0.0  # mean xbar
xsigh = sigx1D  # rms sigma in xbar

y_central = 0.0  # mean ybar
ysigh = sigy1D  # rms sigma in ybar

px_central = 0.0  # mean pxbar
pxsigh = 0.01  # rms sigma in pxbar

py_central = 0.0  # mean pybar
...
z2pos = arange(0, nslices) * dz2 + dz2 / 2.0  # mean z2 coordinate
gammam = np.ones(nslices) * gamma_central + 1.0 * np.cos(
    2.0 * np.pi * (z2pos**0.2) * z2pos
)  # mean gamma (Lorentz factor), added modulation
...
gammasig = (
    np.ones(nslices) * gammasigh
)  # rms sigma in gamma (in units of gamma, NOT gamma / gamma0!!!)
...
chdist = np.ones(
    nslices
)  # Initialize charge distribution function with flat top - all ones
nwaves4sigtail = 10.0
...
chdist[inds] = np.exp(-((z2pos[inds] - taillen) ** 2.0) / 2.0 / tailsig**2.0)

# back tail, at max z2
inds = np.where(z2pos > z2pos[-1] - taillen)
chdist[inds] = np.exp(
    -((z2pos[inds] - (z2pos[-1] - taillen)) ** 2.0) / 2.0 / tailsig**2.0
)
...
f.write(
    ...
    + f"{dz2:<24.15E}"
    + "aw = "
    + f"{puffVars.aw:<24.15E}"
    + "lambdaw = "
    + f"{puffVars.lw:<24.15E}"
    + "lambdar = "
    + f"{puffVars.lr:<24.15E}"
    + "\n"
)
f.write(
    "rho = "
    + f"{puffVars.rho:<24.15E}"
    + "sigx1D = "
    + f"{sigx1D:<24.15E}"
    + "sigy1D = "
    + f"{sigy1D:<24.15E}"
