...
class puffData:
    ...
    def __init__(self):
        ...
        self.powScale = (
            ...
            * self.eps0
            * np.square(
                (self.gamma0 * self.me * np.square(self.c0))
    ...
    def genParams(self):
        ...
        sbetazT = np.sqrt(self.gamma0**2.0 - 1.0 - awrms**2.0) / self.gamma0
        self.eta = (1.0 - sbetazT) / sbetazT
        self.lr = self.eta * self.lw
        self.kappa = self.aw / 2.0 / self.rho / self.gamma0
        self.lg = self.lw / 4.0 / np.pi / self.rho
        self.lc = self.lr / 4.0 / np.pi / self.rho
        self.npkbar = (
            self.lg
            * self.lc**2.0
            * self.eps0
            ...
            * self.gamma0**3.0
            * self.rho**3.0
            * (4.0 * self.c0 * 2.0 * np.pi / self.lw / self.aw) ** 2.0
        )
        ...
        self.lrbar = 4.0 * np.pi * self.rho

    def getRho(self, npk):
        wp = np.sqrt((self.qe**2.0 * npk) / (self.eps0 * self.me))  # plasma frequency

        self.rho = (1 / self.gamma0) * (
            ((self.aw * wp) / (4.0 * self.c0 * (2.0 * np.pi / self.lw))) ** (2.0 / 3.0)
        )
