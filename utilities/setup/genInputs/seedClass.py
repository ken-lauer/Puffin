...
class sd:
    ...
    pkx = np.array([0.0])  # Peak of seed in A_x
    pky = np.array([0.0])  # Peak of seed in A_y
    ssig = np.array([1.0, 1.0, 1.0])
    qFlat = [True]
    meanz2 = np.array([0.0])
    qFRoundEj = [True]
    sigFEj = np.array([0.5])
    ...
    def wrseed(self, f, no):
        ...
        f.write("!\n")
        f.write(
            "! Describes externally injected radiation seeds for the FEL in Puffin.\n"
        )
        f.write(
            "! Please refer to POP-REF for an explanation of the scaled variables\n"
        )
        ...
        f.write(
            "! dtype  - Input type - choose from 'simple', for analtyically described"
        )
        ...
        f.write(
            "! ph_sh - Array containing phase shifts (between 0:2pi) for each seed\n"
        )

        f.write(
            "! sA0_X - Array containing scaled magnitudes for each seed in the x-polarization.\n"
        )
        f.write(
            "! sA0_Y - Array containing scaled magnitudes for each seed in the y-polarization.\n"
        )

        f.write(
            "! sSigmaF - gaussian std dev in each dimension - x, y, then z2. If multiple seeds\n"
        )
        f.write(
            "!           are used, then order inputs as *all* std devs in x, then y, then z2.\n"
        )

        f.write(
            "! qFlatTop - Array containing logicals, describing whether to use a flat top profile\n"
        )
        f.write("!           for each seed in the z2 (longitudinal) dimension.\n")

        f.write(
            "! meanZ2 - Array containing mean (center) of each seed in the longitudinal dimension.\n"
        )

        f.write(
            "! qRndFj_G - Array of logicals, one for each seed. If .true., and using \n"
        )
        f.write(
            "!            a flat-top seed, then seed will be rounded off with a Gaussian \n"
        )
        f.write("!            'tail' at each end. \n")

        f.write(
            "! sSigFj_G - Array of rms standard deviations for the tails, if used (see qRndFj_G). \n"
        )
        f.write(
            "!            One standard deviation for each seed - the same s.d. is used for each \n"
        )
        f.write("!            end. of the seed \n")

        f.write(
            "! qMatchS_G - Array of logicals, one for each seed. If .true., then the seed rms \n"
        )
        f.write(
            "!            s.d. in x and y will be altered to that of the input beams. \n"
        )
