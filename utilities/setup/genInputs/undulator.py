...
class undulator:
    def __init__(
        ...
        if undtype == None:
            ...
            self.uy = 1.0
            self.kbxn = (
                scale.aw / 2.0 / np.sqrt(2) / scale.rho / scale.gamma0
            )  # 'natural' focusing wavenumber
            self.kbyn = (
                scale.aw / 2.0 / np.sqrt(2) / scale.rho / scale.gamma0
            )  # 'natural' focusing wavenumber
            ...
            self.kbxn = scale.aw / np.sqrt(2.0 * scale.eta) / scale.gamma0 * kxu
            self.kbyn = scale.aw / np.sqrt(2.0 * scale.eta) / scale.gamma0 * kyu
        elif undtype == "":
            if ux == None:
            ...
            self.kbxn = self.ux * scale.aw / 2.0 / np.sqrt(2) / scale.rho / scale.gamma0
            self.kbyn = self.uy * scale.aw / 2.0 / np.sqrt(2) / scale.rho / scale.gamma0
    ...
    def getMatrix(self, scale):
        ...
        kbx = np.sqrt(self.kbxn**2.0 + self.kbxSF**2.0)  # rms kbeta
        kby = np.sqrt(self.kbyn**2.0 + self.kbySF**2.0)  # rms kbeta
        ...
        if kbx > 0.0:
            ...
            matx[0][1] = 1.0 / kbx * np.sin(kbx * self.undlen)
        ...
        if kby > 0.0:
            ...
            maty[0][1] = 1.0 / kby * np.sin(kby * self.undlen)
