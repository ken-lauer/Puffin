...
if undtype == "planepole":
    ...
    awrms = aw / np.sqrt(2.0) * np.sqrt(ux**2 + uy**2)
...
sigRound_lam = np.array(
    [6]
)  # Sigma of gaussian used to round off the flat top edges, in resonant wavelengths
bEnOscMag = np.array([0])  # Magnitude of oscillation on beam energy | Units of gamma
bEnOscFr = np.array(
    [7e3]
)  # Frequency (2pi/lambda, where lambda in units of ct) of oscillation on beam energy
# E = 300e6            # Beam energy
...
gammaj = Ej / (
    m_e * pow(c, 2) / q_e
)  # [ij / ( (m_e * pow(c,2) / q_e) ) for ij in Ej] # Rel. factor
...
if qHardEdgeX == 1:
    ...
    r_av = np.sqrt(2.0) * sig_av  # Gaussian dist in x and y
...
rho = (
    1.0 / gamma * np.power((aw * wp[0] / (4.0 * c * k_w)), (2.0 / 3.0))
)  # FEL parameter
...
beta = (
    sqrt(np.power(gamma, 2) - 1.0 - np.power(awrms, 2)) / gamma
)  # Average velocity over c
...
Anoise = (
    6 * sqrt(pi) * rho / (NL * sqrt(log(NL / rho)))
)  # Spontaneous noise estimate (in scaled units)
...
if undtype == "planepole":
    kbxn = 0.0
    kbyn = aw / 2.0 / np.sqrt(2) / rho / gamma  # 'natural' focusing wavenumber

elif undtype == "curved":
    kxu = np.sqrt(eta / 8.0 / rho**2)
    kyu = np.sqrt(eta / 8.0 / rho**2)
    kbxn = aw / np.sqrt(2.0 * eta) / gamma * kxu
    kbyn = aw / np.sqrt(2.0 * eta) / gamma * kyu

elif undtype == "helical":
    kbxn = aw / 2.0 / np.sqrt(2) / rho / gamma  # 'natural' focusing wavenumber
    kbyn = aw / 2.0 / np.sqrt(2) / rho / gamma  # 'natural' focusing wavenumber
...
f.write("!\n")
f.write(
    "! qOneD                      If TRUE, model 1D FEL, with only 1 node and 1 macroparticle in transverse dimensions\n"
)
...
f.write(
    "! qFocussing                 if strong intra-undulator focussing is included in the transverse plane\n"
)
f.write(
...
f.write(
    "! qDumpEnd                   If .TRUE., the do full data dump at end of simulation\n"
)
f.write("! qInitWrLat\n")
f.write("!\n")
f.write("!\n")
...
f.write(
    "! sEThreshold   Below the threshold level(%) * the average of real electrons are removed(ignored)\n"
)
